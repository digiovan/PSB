...
for p in planes:
    for r in rings:
        ...
        graph_2014, ele = getGraphMADX('madx.%s.%surad_default/' % ('2015', kick), opts.CORR, kick, p, r, '2014')
        ...
        graph_2014_offsets, ele = getGraphMADX('madx.%s.%surad_polefaceangles_added/' % ('2015', kick), opts.CORR, kick, p, r, '2014')
        root.SetOwnership(graph_2014_offsets,False)

        graph_2014_offsets.SetLineColor(629)#root.kBlue+2)
        graph_2014_offsets.SetLineStyle(2)
        graph_2014_offsets.SetLineWidth(3)

        ##graph_2014_kr, ele = getGraphMADX('madx.%s.%surad_kr/' % ('2014', kick), opts.CORR, kick, p, r, '2014')
        graph_2014_kr, ele = getGraphMADX('madx.%s.%surad_after_matching/' % ('2015', kick), opts.CORR, kick, p, r, '2014')
        root.SetOwnership(graph_2014_kr,False)

        graph_2014_kr.SetLineColor(root.kAzure+2)
        #graph_2014_kr.SetLineStyle(2)
...
for p in planes:
    for r in rings:
       ...
       # graph_2014.Draw("L")
        ...
        leg = root.TLegend(0.145, 0.748, 0.645, 0.917)
        leg.SetBorderSize(1)
        leg.SetFillColor(0)
        ...
        leg.AddEntry(graph_2014_offsets,   " MADX Sim 2015 (%d #murad)" % kick,     "l")
        leg.AddEntry(graph_2014_kr,   " MADX Sim 2015 after matching (%d #murad)" % kick,     "l")
        leg.Draw()
